...
### 3.4 Bảng "Danh sách thiết bị báo giá nháp" (`quoteItems`) — ĐÃ GIẢI QUYẾT (2026-07-21), không phải gap DB thật

> **Cập nhật 2026-07-21 (theo yêu cầu người dùng) — nhận định gốc bên dưới SAI.** Gap này không nằm ở
> `survey_reports` (đúng là bảng đó không có cột nào lưu thiết bị/giá nháp) — nhưng dữ liệu này **không
> cần phải lưu ở `survey_reports`**, vì nó đã tồn tại thật ở nơi khác: **báo giá (quotation) liên kết
> với đơn của báo cáo khảo sát đó**. Đường đi: `report.orderId` → `GET /api/v1/orders/:orderId` (trả về
> `quotationId`, đã xác nhận qua `curl`) → `GET /api/v1/quotations/:quotationId` (trả về `items[]` đầy
> đủ: `itemName`/`categoryName`/`unit`/`quantity`/`price`/`lineTotal` — đúng khớp mọi cột bảng UI cần,
> kể cả tổng `totalAmount`). **Không cần bảng mới, không cần Backend làm gì thêm.** Đã sửa
> `SurveyDetailDrawer.tsx` gọi 2 API trên tuần tự khi mở drawer, bỏ hẳn `MOCK_QUOTE_ITEMS` + nhãn "(dữ
> liệu minh họa)", thêm 3 trạng thái hiển thị (đang tải / đơn chưa liên kết báo giá / lỗi tải). Mục
> 3.6 điểm 2 và bảng tổng hợp mục 8 dòng liên quan đã cập nhật theo — không cần ghi việc này vào
> `docs/more-require.md` nữa (đã đánh dấu giải quyết ở mục (t) file đó).
>
> Lưu ý: cách này chỉ đúng khi đơn **đã** có báo giá liên kết (`orders.quotationId` khác `null`) — nếu
> khảo sát diễn ra ở giai đoạn báo giá còn nháp/chưa gắn đơn thật, đường liên kết `orderId → quotationId`
> chưa tồn tại (đúng vấn đề đã ghi ở `docs/kehoachvaphancong_api.md` mục 8.1 — Order chỉ được tạo sau khi
...
`report.quoteItems` — cách giải quyết ở trên (đi qua `quotationId` của đơn) áp dụng được y hệt nếu màn
đó cần làm lại sau này.
...
2. ~~Làm rõ nghiệp vụ "thiết bị báo giá nháp" (`quoteItems`)~~ — **ĐÃ GIẢI QUYẾT (2026-07-21), không
   cần Backend làm gì** — dữ liệu lấy từ báo giá liên kết với đơn (`orders.quotationId` →
   `GET /quotations/:id`), xem mục 3.4 đã cập nhật.
